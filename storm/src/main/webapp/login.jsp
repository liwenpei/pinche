...
<body class="login">
	...
	<!-- BEGIN LOGO -->
	<div class="logo">
		<a href="index.html"> <img alt="" src="images/logo.jpg" width="70">
		</a>
	</div>
	<!-- END LOGO -->
	<!-- BEGIN LOGIN -->
		...
		<form class="login-form" action="login.do" method="post">
			...
			<div class="form-group">
				...
				<label class="control-label visible-ie8 visible-ie9">用户名</label> <input
					...
					type="text" autocomplete="off" placeholder="用户名" id="user_id"
					name="user_id" />
			</div>
			<div class="form-group">
				<label class="control-label visible-ie8 visible-ie9">密码</label> <input
					class="form-control form-control-solid placeholder-no-fix"
					type="password" autocomplete="off" placeholder="密码" id="password"
					name="password" />
			</div>
			<div class="form-actions">
				<button type="submit" class="btn btn-success uppercase">登录</button>
				<label class="rememberme"> <input type="checkbox" id="remCb"
					name="remember1" onchange="remember()"  />记住密码
				</label> <input type="hidden" id="remFlag" name="remFlag" />
			</div>
		</form>
		<!-- END LOGIN FORM -->
	</div>
...
<script>
	...
	function initCookie() {
		//记住密码功能
		var str = getCookie("loginInfo").trim();
		if (str != "") {
			...
			$("#remFlag").val("1");
			$("#remCb").attr("checked", true);
		} else {
			$("#remFlag").val("");
			$("#remCb").attr("checked", false);
		}
	...
	function logoutCookie() {
		...
		document.cookie = "loginInfo" + "=; expire=" + date.toGMTString()
				+ "; path=/";
	}
